...
public class FilePartitionedSnapshotLogManager extends PartitionedSnapshotLogManager<FileSnapshot> {
   ...
   * situations: If the tsfile is empty at this time (only the metadata is registered, but the file
   * has not been written yet), then the timeseries schema snapshot can still be generated and sent
   * to the followers.
   *
   * @throws IOException
   */
  private void collectTsFilesAndFillTimeseriesSchemas() throws IOException {
    // 1. first collect tsfile
    collectTsFiles();

    //2. second register the measurement
