 ...
 */
public class AsyncHeartbeatDataClient extends AsyncClient {

  private static final Logger logger = LoggerFactory.getLogger(AsyncHeartbeatDataClient.class);

  private Node node;
  private AsyncClientPool pool;

  public AsyncHeartbeatDataClient(TProtocolFactory protocolFactory,
      TAsyncClientManager clientManager, Node node, AsyncClientPool pool) throws IOException {
    // the difference of the two clients lies in the port
    super(protocolFactory, clientManager, new TNonblockingSocket(node.getIp(), node.getHeartbeatDataPort()
  ...
  public static class FactoryAsync implements AsyncClientFactory {
    ...
    public RaftService.AsyncClient getAsyncClient(Node node, AsyncClientPool pool) throws IOException {
      ...
      return new AsyncHeartbeatDataClient(protocolFactory, manager, node, pool);
    }
  }
